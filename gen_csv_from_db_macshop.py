...
def get_price(content):
    ...
    body=soup.findAll("div", {"id":"main-content"})
    for b in body:
        content=b.get_text()
...
def gen_csv_from_db(db_path, output_name, loglevel):
    ...
    with open(output_name, 'wb') as _csv:
        ...
        for _p in _all_posts:
            ...
            _title=_p[2]
            _title_enc=_title.encode('utf-8')
            _title_split = re.match(ur"\[(.*[\u2E80-\u9FFF]+)\](.*)", _title_enc.decode('utf-8'))
            ...
            try:
                if _title_split.group(2):
                    _title_subtitle = _title_split.group(2).strip().encode('utf-8')
                else:
                    _title_subtitle=""
                ...
                _title_subtitle = ""
            try:
                _title_how = _title_split.group(3).strip().encode('utf-8')
            except:
                _title_how = "".encode('utf-8')
            try:
                _date = _p[0].strip().encode('utf-8')
            except:
                _date = ""
            try:
            ...
            _row.append(_date)
            _row.append(_author)
            _row.append(_title_type.encode('utf-8'))
            _row.append(_title_subtitle)
            #_row.append(_title_how.encode('uft-8'))
            _row.append(get_push_cnt(_p[5]))
            _row.append(get_price(_p[5]).encode('utf-8'))
            _row.append(_p[3].strip().encode('utf-8'))
